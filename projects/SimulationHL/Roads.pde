  ...
  public void readMesh(ArrayList<Cells> mesh, PGraphics canvasReader) {
    for (Cells cell : mesh) {
      ...
      if (result) {
        ...
        if (cell.colorName.equals("red")) { 
          latent = new AffectedArea(roads, str(roads.count()), "affectedArea", "affectedArea", new PVector(newW, newH), 250, 120);
          latent.ownColor = col;
        }
        if (cell.colorName.equals("yellow")) {
          latent = new AffectedArea(roads, str(roads.count()), "affectedArea", "affectedArea", new PVector(newW, newH), 200, 120);
          latent.ownColor = col;
        }
        if (cell.colorName.equals("green")) {
          latent = new AffectedArea(roads, str(roads.count()), "affectedArea", "affectedArea", new PVector(newW, newH), 150, 120);
          latent.ownColor = col;
        }
