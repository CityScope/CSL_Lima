...
public class Cells {
   ...
   */
  public Cells(PVector startPoint, int size, ArrayList<Color> colors) {
    START = startPoint;
    SIZE = size;
    COLORS = colors;
    setCounter();
  ...
  /**
   * Assigns a standard color to every group of an image's pixels in the same color range
...
/**
 * PatternCells - Class which represents a 2x2 arrangement of Cells objects. Its ID depends on the color of the Cell objects inside it.
...
public class PatternCells {
   ...
   */
  public PatternCells(PVector startPoint, int size, PatternBlocks pBlocks) {
    START = startPoint;
    SIZE = (int) size;
    PBLOCKS = pBlocks;
    create(PBLOCKS.getColors());
  }
  ...
  private void create(ArrayList<Color> colors) {
    CELLS.add(new Cells(START, SIZE/2, colors));
    CELLS.add(new Cells(new PVector(START.x + SIZE/2, START.y), SIZE/2, colors));
    CELLS.add(new Cells(new PVector(START.x + SIZE/2, START.y + SIZE/2), SIZE/2, colors));
    CELLS.add(new Cells(new PVector(START.x, START.y + SIZE/2), SIZE/2, colors));
  }
...
public class Mesh {
  ...
  private void create(PatternBlocks pBlocks) {
    ...
    for (int h = 0; h < GRID.length; h++) {
      for (int w = 0; w < GRID.length; w++) {
        float pointx = w * SIZE;
        float pointy = h * SIZE;
        tempCells.add(new PatternCells(new PVector(pointx, pointy), SIZE, pBlocks));
      }
    }
    PCELLS = tempCells;
  }
  ...
  /**
   * Gets the vakue of the GRID attribute
   * @returns: PVector[][]  Value of GRID
   */
  public PVector[][] getGrid() {
    return GRID;
  }
