...
public class PatternBlocks {
   ...
   */
  public PatternBlocks(ArrayList<JSONArray> options, ArrayList<Color> colors, int canvasSize, int blockSize) {
    OPTIONS = options;
    COLORS = colors;
    BLOCKSIZE = blockSize;
    ...
    for (int j = 0; j < yStep; j++) {
      for (int i = 0; i < xStep; i++) {
        if (index < OPTIONS.size()) {
          ...
          Block firstBlock = new Block(BLOCKSIZE, coordinatesFirstBlock, COLORS, nameFirstBlock);          

          PVector coordinatesSecondBlock = new PVector(initP.x + BLOCKSIZE, initP.y);
          String nameSecondBlock = (String) colorNames.get(1); 
          Block secondBlock = new Block(BLOCKSIZE, coordinatesSecondBlock, COLORS, nameSecondBlock);
          ...
          Block thirdBlock = new Block(BLOCKSIZE, coordinatesThirdBlock, COLORS, nameThirdBlock);

          PVector coordinatesFourthBlock = new PVector(initP.x, initP.y + BLOCKSIZE);
          String nameFourthBlock = (String) colorNames.get(3); 
          Block fourthBlock = new Block(BLOCKSIZE, coordinatesFourthBlock, COLORS, nameFourthBlock); 
  ...
  public JSONArray saveColors() {
    JSONArray colorLimits = new JSONArray();
    for (Color c : COLORS) {
      colorLimits.append(c.saveConfiguration());
    }
    return colorLimits;
  }
}
...
public class Patterns extends PApplet {
   ...
   */
  public Patterns(JSONObject calibrationParameters) {
  ...
  private void load(JSONObject calibrationParameters) {
    ...
    for (int i = 0; i < colorLimits.size(); i++) {
      ...
      if (acronym.equals("W")) {
        colors.add(new White(c));
      } else if (acronym.equals("BK")) {
        colors.add(new Black(c));
      } else {
        colors.add(new Other(c));
      }
    }
